...
def BasicTest(nonces=[9999], latencies=[0], clients_to_start=[0], miners_to_start=[0]):
    ...
    # Start all clients
    running_clients, client_args = start_some_processes(
        client_args, args_to_start=clients_to_start, process_type="Client"
    )
    assert len(client_args) == len(nonces) - len(clients_to_start)
    assert len(running_clients) == len(clients_to_start)

    # Start all miners
    running_miners, miner_args = start_some_processes(
        miner_args, args_to_start=miners_to_start, process_type="Miner"
    )
...
if __name__ == "__main__":
    ...
    BasicTest(
        nonces=[999, 99999], latencies=[0], clients_to_start=[0, 1], miners_to_start=[0]
    )
